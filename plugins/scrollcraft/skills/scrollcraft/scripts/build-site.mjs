...
function signupFormHtml(f, s, idx, centered) {
  ...
  return `<form class="sc-signup" method="post" action="${esc(f.action)}" target="_blank" rel="noopener" style="display:flex; flex-wrap:wrap; gap:0.5rem; width:min(520px, calc(100vw - 4rem)); justify-content:${centered ? "center" : "flex-start"}; margin:0.5rem ${centered ? "auto" : "0"} 0;"><label for="${id}" class="sc-visually-hidden">Email address</label><input id="${id}" type="email" name="${esc(f.emailName)}" required autocomplete="email" placeholder="you@example.com" style="flex:1 1 240px; min-width:0; min-height:48px; padding:0 1rem; border-radius:${radius}; border:1px solid rgba(255,255,255,0.35); background:rgba(0,0,0,0.35); color:inherit; font:inherit; font-size:1rem;" />${hidden}<button type="submit" style="flex:0 0 auto; min-height:48px; padding:0 1.5rem; border:0; border-radius:${radius}; background:${safeCss(s.accentColor || "var(--sc-accent, #7c3aed)")}; color:#fff; font:inherit; font-weight:600; font-size:1rem; cursor:pointer;">${esc(s.signupButton || "Join the waitlist")}</button></form>`;
}
  ...
  return sections.map((s, idx) => {
    ...
    return `    <section class="scroll-section" style="height:${height}px; position:relative; z-index:10;">
      <div class="section-sticky" style="position:sticky; top:0; height:100vh; display:flex; align-items:${safeCss(s.align || L.align)}; justify-content:${safeCss(s.justify || L.justify)}; overflow:hidden;">
        <div class="section-content" data-reveal="${reveal}" style="${scrim > 0 ? `background:radial-gradient(ellipse 120% 100% at 50% 50%, rgba(0,0,0,${scrim}) 0%, rgba(0,0,0,${(scrim * 0.72).toFixed(3)}) 45%, rgba(0,0,0,0) 78%); ` : ""}text-align:${safeCss(s.textAlign || L.textAlign)}; padding:${L.pad}; max-width:${L.maxWidth}px; box-sizing:content-box; transition:opacity 0.6s cubic-bezier(0.25,0.46,0.45,0.94),transform 0.6s cubic-bezier(0.25,0.46,0.45,0.94),clip-path 0.7s cubic-bezier(0.25,0.46,0.45,0.94);">
${parts.map((p) => "          " + p).join("\n")}
        </div>
      </div>
    </section>`;
